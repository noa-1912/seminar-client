...
export default function PrivateInvitations() {
  ...
  const openApplyDialog = (invitation) => {
    setApplyTarget(invitation);
    setCoverLetter('');
    setResumeUrl('');
    setApplyError(null);
  };
  ...
  const handleSubmitApplication = async () => {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        throw new Error('יש להתחבר כסטודנטית כדי להגיש מועמדות.');
      }
      if (res.status === 400) {
        const text = await res.text();
        throw new Error(text || 'הבקשה לא התקבלה. ייתכן שכבר הגשת למשרה זו.');
      }
      if (!res.ok) {
        throw new Error(`אירעה שגיאה בהגשת המועמדות (${res.status}).`);
      }

      setAppliedJobIds((prev) => {
        const next = new Set(prev);
        next.add(applyTarget.jobId);
        return next;
      });
      setSuccessMessage(`מועמדותך למשרה "${applyTarget.jobTitle}" הוגשה בהצלחה.`);
      setApplyTarget(null);
    } catch (err) {
      setApplyError(err.message || 'אירעה שגיאה לא ידועה.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      >
        ...
        <DialogContent>
          <DialogContentText sx={{ mb: 2 }}>
            את עומדת להגיש מועמדות למשרה שהוזמנת אליה אישית. ניתן להוסיף מכתב מקדים וקישור לקובץ קורות חיים (שניהם אופציונליים).
          </DialogContentText>
          ...
          <Stack spacing={2}>
            ...
            />
            <TextField
              label="קישור לקורות חיים (אופציונלי)"
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={closeApplyDialog} disabled={submitting} color="inherit">
            ביטול
          </Button>
          <Button
          ...
          >
            {submitting ? 'שולחת...' : 'שליחת מועמדות'}
          </Button>
        </DialogActions>
      </Dialog>
